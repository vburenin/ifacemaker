...
// fixtureConfig is the per-fixture config read from __fixtures__/<name>/fixture.json.
...
// TestFixtures runs ifacemaker for each fixture under __fixtures__/<name>/ and
// compares generated output to __fixtures__/<name>/output/.
func TestFixtures(t *testing.T) {
	...
	for {
		...
		if parent == root {
			t.Skip("fixtures test requires running from module root")
			return
		}
		root = parent
	}

	fixturesDir := filepath.Join(root, "__fixtures__")
	entries, err := os.ReadDir(fixturesDir)
	if err != nil {
		if os.IsNotExist(err) {
			t.Skip("no __fixtures__ directory")
			return
		}
		require.NoError(t, err)
	}
	...
	for _, e := range entries {
		...
		t.Run(name, func(t *testing.T) {
			...
			outputDir := filepath.Join(fixtureDir, "output")

			cfgPath := filepath.Join(fixtureDir, "fixture.json")
			cfgData, err := os.ReadFile(cfgPath)
			require.NoError(t, err, "fixture %s: read fixture.json", name)

			var cfg fixtureConfig
			require.NoError(t, json.Unmarshal(cfgData, &cfg), "fixture %s: parse fixture.json", name)

			var files []string
			for _, f := range cfg.Files {
				files = append(files, filepath.Join(inputDir, f))
			}
			for _, p := range files {
				_, err := os.Stat(p)
				require.NoError(t, err, "fixture %s: input file %s", name, p)
			}

			got, err := maker.Make(maker.MakeOptions{
				Files:        files,
				StructType:   cfg.Struct,
				IfaceName:    cfg.Iface,
				PkgName:      cfg.Pkg,
				WithPromoted: cfg.Promoted,
				Comment:      "Code generated by ifacemaker; DO NOT EDIT.",
				IfaceComment: cfg.Iface + " ...",
			})
			require.NoError(t, err, "fixture %s: run ifacemaker", name)

			expectedPath := filepath.Join(outputDir, cfg.Output)
			expected, err := os.ReadFile(expectedPath)
			require.NoError(t, err, "fixture %s: read expected output", name)

			require.Equal(t, strings.TrimSpace(string(expected)), strings.TrimSpace(string(got)),
				"fixture %s: generated output differs from __fixtures__/%s/output/%s", name, name, cfg.Output)
		})
	}
}
